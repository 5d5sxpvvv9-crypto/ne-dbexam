...
# olefile·추출은 I/O·네이티브 비중이 있어 워커를 약간 올림. Windows COM 다중 인스턴스 부담 시 MAX_CONCURRENT=4~6 권장.
_default_workers = min(12, max(5, (os.cpu_count() or 4) * 2))
MAX_CONCURRENT = int(os.environ.get("MAX_CONCURRENT", str(_default_workers)))
UPLOAD_CHUNK_SIZE = 1024 * 1024  # 1MB
STATUS_BATCH_LIMIT = 200
CLEANUP_MINUTES = 60
...
async def _save_upload_streaming(file: UploadFile, filepath: str) -> None:
    """전체 파일을 메모리에 올리지 않고 디스크에 기록. 크기 상한은 스트리밍 중 검사."""
    total = 0
    with open(filepath, "wb") as out:
        while True:
            ...
            if next_total > MAX_FILE_SIZE:
                raise HTTPException(
                    status_code=413,
                    detail=f"파일 크기 초과: {next_total / 1024 / 1024:.1f}MB (최대 50MB)",
                )
            total = next_total
            out.write(chunk)
...
async def upload_files(files: List[UploadFile] = File(...)):
    ...
    for file in files:
        ...
        # 임시 파일 저장 (스트리밍 — 대용량 시 메모리 피크 감소)
        task_id = str(uuid.uuid4())[:8]
        safe_filename = f"{task_id}_{file.filename}"
        filepath = os.path.join(TEMP_DIR, safe_filename)

        try:
            await _save_upload_streaming(file, filepath)
        ...
        asyncio.get_running_loop().run_in_executor(
            executor, _process_hwp_file, task_id, filepath, file.filename
        )
...
async def get_task_status(task_id: str):
    ...
    return _public_task_status(task)


@app.post("/api/status/batch")
async def get_tasks_status_batch(body: StatusBatchRequest):
    """대기·처리 중 작업 상태를 한 번에 조회 (폴링 시 HTTP 왕복 감소)."""
    ids = body.task_ids
    if len(ids) > STATUS_BATCH_LIMIT:
        raise HTTPException(
            status_code=400,
            detail=f"task_ids는 최대 {STATUS_BATCH_LIMIT}개까지 요청할 수 있습니다.",
        )
    seen = set()
    tasks_out = []
    for tid in ids:
        if tid in seen:
            continue
        seen.add(tid)
        if tid not in file_tasks:
            continue
        tasks_out.append(_public_task_status(file_tasks[tid]))
...
async def retry_task(task_id: str):
    ...
    asyncio.get_running_loop().run_in_executor(
        executor, _process_hwp_file, task_id, filepath, task["filename"]
    )
...
async def startup_event():
    ...
    logger.info(f"동시 처리 워커: {MAX_CONCURRENT} (환경변수 MAX_CONCURRENT로 변경 가능)")
