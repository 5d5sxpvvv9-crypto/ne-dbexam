...
_OBJECTIVE_STEM_RE = re.compile(
    r'(?:적절한|알맞은|올바른|자연스러운|옳은|바른|맞는|틀린|어색한|잘못된|바르게|어울리는|일치하는|일치하지|옳지\s*않은|없는|않는)\s*것'
    r'|(?:배열한|나열한|짝지은|연결한|짝지어진|고른)\s*것'
    r'|(?:들어가기에|들어갈)\s*(?:가장\s*)?적절한\s*곳'
    r'|고르[시면]|모두\s*몇\s*개|개수는|개인가'
    r'|(?:적절한|알맞은)\s*(?:말|곳|단어|문장)'
    r'|사용되지\s*않는|들어갈\s*수\s*없는|답할\s*수\s*없는|답할\s*수\s*있는'
    r'|번째\s*(?:순서의\s*)?단어는|패스워드는'
)


def _is_objective_by_stem(question_text: str) -> bool:
    """발문 패턴으로 객관식 여부 추정 (선지 미검출 보완)"""
    first_line = question_text.split('\n')[0]
    clean = _clean_line_suffixes(first_line)
    if _OBJECTIVE_STEM_RE.search(clean):
        return True
    return False
...
def _build_answer_map_from_endnotes(
    endnote_answers: List[Tuple[str, str]],
    lines: List[str],
    expected_numbers: List[int],
    number_positions: Dict[int, int],
) -> Dict[int, str]:
    """pyhwp에서 추출한 미주 정답을 문항 번호에 매핑.

    매핑 전략:
      1) 미주의 부모 문단 텍스트와 문항 텍스트 비교 (부분 문자열 포함 검사)
      2) 텍스트 매칭 50% 미만 시 문서 순서(sequential) 매핑
    """
    if not endnote_answers or not expected_numbers:
        return {}

    answer_map: Dict[int, str] = {}

    # 각 문항의 텍스트 수집 (번호 제거, 공백 정규화, 여러 줄 포함)
    q_texts: Dict[int, str] = {}
    sorted_nums = sorted(expected_numbers)
    for i, qnum in enumerate(sorted_nums):
        line_idx = number_positions.get(qnum, -1)
        if line_idx < 0 or line_idx >= len(lines):
            continue
        text_parts = []
        next_line = number_positions.get(sorted_nums[i + 1], len(lines)) if i + 1 < len(sorted_nums) else len(lines)
        for j in range(line_idx, min(line_idx + 3, next_line)):
            if j < len(lines):
                text_parts.append(lines[j].strip())
        raw = ' '.join(text_parts)
        stripped = re.sub(r'^\d+\s*[.)]\s*', '', raw)
        q_texts[qnum] = re.sub(r'\s+', '', stripped)
    ...
    def _similarity(a: str, b: str) -> float:
        ...
        a_short = a[:50]
        b_short = b[:50]
        if a_short in b_short or b_short in a_short:
            return 1.0
        common = 0
        for ch in set(a_short):
            common += min(a_short.count(ch), b_short.count(ch))
        return common / max(len(a_short), len(b_short))

    # 전략 1: 부모 문단 텍스트 매칭
    matched: Set[int] = set()
    for para_text, en_answer in endnote_answers:
        if not para_text:
            continue
        clean_para = re.sub(r'\s+', '', para_text)[:50]
        if len(clean_para) < 3:
            continue

        best_qnum = None
        best_score = 0.0
        for qnum, qtxt in q_texts.items():
            if qnum in matched:
                continue
            score = _similarity(clean_para, qtxt[:80])
            if score > best_score:
                best_score = score
                best_qnum = qnum

        if best_qnum is not None and best_score >= 0.4:
            answer_map[best_qnum] = en_answer
            matched.add(best_qnum)

    # 전략 2: 순서 매핑 (텍스트 매칭 50% 미만 시)
    if len(answer_map) < len(endnote_answers) * 0.5:
        answer_map_seq: Dict[int, str] = {}
        en_idx = 0
        for qnum in sorted_nums:
            if en_idx >= len(endnote_answers):
                break
            _, en_answer = endnote_answers[en_idx]
            answer_map_seq[qnum] = en_answer
            en_idx += 1
        if len(answer_map_seq) > len(answer_map):
            logger.info("미주 순서 매핑 사용 (텍스트 매칭 부족)")
            answer_map = answer_map_seq
...
def _recover_missing_question(
    ...
    q.question_format = "객관식" if (choice_lines or _is_objective_by_stem(question_text)) else "서술형"
    q.notes += "; [3단계 복구]"
...
def extract_questions(
    full_text: str,
    filename: str = "",
    endnote_answers: Optional[List[Tuple[str, str]]] = None,
) -> ExtractionResult:
    ...
    Args:
        endnote_answers: pyhwp에서 추출한 미주 정답 [(문단텍스트, 정답), ...]
    """
    ...
    # ── 미주(endnote) 정답 보충 ──
    if not answer_map and endnote_answers:
        answer_map = _build_answer_map_from_endnotes(
            endnote_answers, lines, expected_numbers, number_positions
        )
        if answer_map:
            result.answer_section_found = True
            logger.info(f"미주(endnote) 정답 매핑 완료: {len(answer_map)}개")
