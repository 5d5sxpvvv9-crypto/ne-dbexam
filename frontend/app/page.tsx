...
export default function HomePage() {
  ...
  const QUESTIONS_FETCH_PARALLEL = 4;
  ...
  // ── questions fetch (완료 작업 여러 개를 병렬로 가져와 미리보기 지연 감소) ──
  const processQuestionsFetchQueue = useCallback(async () => {
    while (questionsFetchQueue.current.length > 0) {
      const batch = questionsFetchQueue.current.splice(0, QUESTIONS_FETCH_PARALLEL);
      ...
      for (let i = 0; i < settled.length; i++) {
        ...
        if (result.status !== "fulfilled") {
          ...
          questionsFetchQueue.current.unshift(taskId);
          hadFailure = true;
          console.warn(`[questions] fetch 실패 — taskId=${taskId} (큐에 재등록)`);
          continue;
        }
        const { qData } = result.value;
  ...
  useEffect(() => {
    ...
    pollingRef.current = setInterval(async () => {
      ...
      try {
        ...
        let rows: StatusRow[] = [];
        const batchRes = await fetch(`${API_BASE}/api/status/batch`, {
          method: "POST",
          headers: { "Content-Type": "application/json" },
          body: JSON.stringify({ task_ids: pending.map((t) => t.task_id) }),
        });

        if (batchRes.ok) {
          const j = await batchRes.json();
          rows = Array.isArray(j.tasks) ? j.tasks : [];
        } else {
          ...
          for (const r of results) {
            if (r.status === "fulfilled" && r.value?.task_id) rows.push(r.value);
          }
        }

        const completedForQuestions: string[] = [];
        for (const data of rows) {
          if (
            data.status === "completed" &&
            (data.total_questions ?? 0) > 0 &&
            !fetchedTaskIdsRef.current.has(data.task_id)
          ) {
            fetchedTaskIdsRef.current.add(data.task_id);
            completedForQuestions.push(data.task_id);
          }
        }
        for (const tid of completedForQuestions) {
          questionsFetchQueue.current.push(tid);
        }

        if (rows.length > 0) {
          setTasks((prev) =>
            prev.map((t) => {
              const row = rows.find((r) => r.task_id === t.task_id);
              if (!row) return t;
              return {
                ...t,
                status: row.status as FileTask["status"],
                total_questions: row.total_questions ?? t.total_questions,
                error: row.error ?? null,
                warnings: Array.isArray(row.warnings) ? row.warnings : t.warnings,
                metadata:
                  row.metadata && typeof row.metadata === "object" ? row.metadata : t.metadata,
                parse_method: row.parse_method ?? t.parse_method,
                parse_time_ms: row.parse_time_ms ?? t.parse_time_ms,
              };
            })
          );
        }
